...
{
    printf("Version 0.%d Build %s %s\n", VERSION_NUMBER, __DATE__, __TIME__);
#ifdef GIT_REPO_VERSION
    printf("Git version: %s\n", GIT_REPO_VERSION);
#endif

    printf("\nUsage:\n");
    printf("    %s [-hds] [-o offset] [-n bytes] [-b base] ", program_name);
#if defined(SPI_RW)
    printf("fpga register [data]\n");
#elif defined(EBI_RW)
    printf("address datasize [data]\n");
#endif

    printf("\nOptions:\n");
    printf("    -h  help, print this help\n");
    printf("    -d  debug, enable debug mode to print debugging messages\n");
    printf("    -s  signed, set signed format for output data (only for dec base)\n");
    printf("    -o  offset, offset for address\n");
    printf("    -n  bytes (1|2|4), set size to data (0xAA 0xAABB 0xAABBCCDD). Default-1.\n");
    printf("    -b  base (2|10|16), set output data format (bin|dec|hex). Default-10.\n");

    printf("\nSupported prefixes for input data:\n");
    printf("    hex:  0x x 0X X 0h h 0H H\n");
    printf("    bin:  0b b 0B B\n");

    printf("\nExamples:\n");
#if defined(EBI_RW)
    printf("    %s 0xAA 1            - read  1 byte\n", program_name);
    printf("    %s 0xAA 1 0xBB       - write 1 byte\n", program_name);
    printf("    %s 0xAA 2 0xBB 0xCC  - write 2 byte\n", program_name);
    printf("    %s 0xAA 2 0xBBCC -n2 - write 2 byte\n", program_name);
#elif defined(SPI_RW)
    printf("    %s 0xAA 1 1            - read 1 byte,  fpga 1\n", program_name);
    printf("    %s 0xAA 0 1 0xBB       - write 1 byte, fpga 1\n", program_name);
    printf("    %s 0xAA 4 2 0xBB 0xCC  - write 2 byte, fpga 4\n", program_name);
    printf("    %s 0xAA 4 2 0xBBCC -n2 - write 2 byte, fpga 4\n", program_name);
#endif
}
...
unsigned long parse_number(char *str)
{
    ...
    {
        printf("Error: unknow data formaton in argument \"%s\"\n", str);
        exit(EXIT_FAILURE);
    }
...
uint32_t parse_data(char *str)
{
    ...
    {
        printf("Error: argument \"%s\" > UINT32_MAX\n", str);
        exit(EXIT_FAILURE);
    }
    else if (options.data_bytes == 2 && data > UINT16_MAX)
    {
        printf("Error: argument \"%s\" > UINT16_MAX\n", str);
        exit(EXIT_FAILURE);
    }
    else if (options.data_bytes == 1 && data > UINT8_MAX)
    {
        printf("Error: argument \"%s\" > UINT8_MAX\n", str);
        exit(EXIT_FAILURE);
    }
...
uint32_t parse_address(char *str)
{
    ...
    {
        printf("Error: address can not be over 0x%X\n", MAX_ADDRESS);
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char **argv)
{
    ...
    {
        printf("Error: data arguments too much or too few\n");
        exit(EXIT_FAILURE);
    }
    ...
    {
        printf("Error: datasize argument over MAXIMUM_ESPI_DATA_SIZE\n");
        exit(EXIT_FAILURE);
    }
    if (package.datasize == 0)
    {
        printf("Error: datasize can not be 0\n");
        exit(EXIT_FAILURE);
    }
    if (package.datasize % options.data_bytes)
    {
        printf("Error: datasize not divided on bytesize (-n)\n");
        exit(EXIT_FAILURE);
    }
#if defined(SPI_RW)
    if (package.spi < 1 || package.spi > 11)  // 1 modem + 2 dos + 8 ppm
    {
        printf("Error: fpga number %d is wrong, should be 1...11\n", package.spi);
        exit(EXIT_FAILURE);
    }
...
#endif
        {
            printf("Error: datasize not equal to the entered data\n");
            exit(EXIT_FAILURE);
        }
    ...
    {
        printf("Error: offset+address+datasize can not be over 0x%X\n", MAX_ADDRESS);
        exit(EXIT_FAILURE);
    }
#elif defined(SPI_RW)
    if (package.address > MAX_ADDRESS)
    {
        printf("Error: offset+address can not be over 0x%X\n", MAX_ADDRESS);
        exit(EXIT_FAILURE);
    }
#endif
    ...
    if (file_desc < 0)
    {
        printf("Can't open device file /dev/espi\n");
        exit(EXIT_FAILURE);
    }
