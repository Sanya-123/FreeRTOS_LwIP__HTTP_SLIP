...
static void microrl_get_complite (microrl_t * pThis) 
{
	...
	if (pThis->cmdline[pThis->cursor-1] == '\0')
		tkn_arr[status++] = "";
    compl_token = pThis->get_completion ((void*)pThis, status, tkn_arr);
...
void new_line_handler(microrl_t * pThis){
	...
	if ((status > 0) && (pThis->execute != NULL))
        pThis->execute ((void*)pThis, status, tkn_arr);
	print_prompt (pThis);
	pThis->cmdlen = 0;
	pThis->cursor = 0;
	memset(pThis->cmdline, 0, _COMMAND_LINE_LEN);
	...
	if (pThis->escape) {
		...
		switch (ch) {
			...
			if (microrl_insert_text (pThis, (char*)&ch, 1)){
                if(!pThis->terminalHasEcho)//если у терминала нету эхо
